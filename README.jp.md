...
- [Claude Code](https://claude.ai/code) CLI がインストール済み（Claude Code プラグイン用）
- [OpenCode](https://opencode.ai) CLI がインストール済み（OpenCode スキル用）
...
#### Claude Code プラグイン（フル機能）

```bash
# ワンクリックインストール - リポジトリをクローン、インストール、検証
...
- ✅ 前提条件を確認（Git）
- ✅ 一時的にリポジトリをクローン
- ✅ プラグインを `~/.claude/plugins/open-reflect` にコピー
- ✅ スクリプトを実行可能に
- ✅ プラグインを有効化
- ✅ キャッシュをクリア
- ✅ インストール状況を表示
...
```bash
# ワンクリックインストール - リポジトリをクローン、スキル、検証
curl -sSL https://raw.githubusercontent.com/gyc567/open-reflect/master/.opencode/scripts/install-opencode-skill.sh | bash
```

**機能：**
- ✅ 前提条件を確認（Git）
- ✅ ローカルファイルを使用またはリポジトリをクローン
- ✅ `~/.config/opencode/skill/` ディレクトリを作成
- ✅ スキルファイルをコピー（SKILL.md + AGENTS.md + rules/）
- ✅ インストールを検証
- ✅ 使用方法の表示
...
| 機能 | Claude Code プラグイン | OpenCode スキル |
|------|----------------------|----------------|
| **自動キャプチャ** | ✅ フル hooks | ⚪ 手動トリガー |
| **キュー管理** | ✅ フル | ⚪ Basic |
| **演化トラッキング** | ✅ 完全な履歴 | ⚪ 静的 |
| **スマート分析** | ✅ トレンドと洞察 | ⚪ なし |
| **コマンド** | `/reflect` | `/reflect` |
| **コマンド** | `/skip-reflect` | `/skip-reflect` |
| **コマンド** | `/view-queue` | ⚪ なし |
...
# スキルファイルを OpenCode 設定 directory にコピー
...
| 問題 | 解決策 |
|------|--------|
| プラグインが読み込まれない | Claude Code を完全に再起動（Cmd+Q / Ctrl+Q） |
| コマンドが認識されない | プラグインファイルを検証：`ls ~/.claude/plugins/open-reflect/` |
| キューが更新されない | hooks を確認：`cat ~/.claude/plugins/open-reflect/hooks/hooks.json` |
| 権限拒否 | スクリプトを実行可能に：`chmod +x scripts/*.sh` |
